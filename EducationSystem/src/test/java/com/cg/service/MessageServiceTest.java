...
import com.cg.services.MessageServiceImp;
...
class MessageServiceTest {
	...
	public void testAddMessage() { // test for adding message
		...
		message.setPostedOn(LocalDate.parse("2020-01-01"));
	...
	public void testViewMessage() { // test for viewing message
		...
		message.setPostedOn(LocalDate.parse("2020-01-01"));
		message.setMessageText("hello all");
	...
	public void testViewAllMessages() { // test for viewing messages
		...
		message1.setPostedOn(LocalDate.parse("2020-02-02"));
		message1.setMessageText("hello all.....");
		List<Message> messageList = new ArrayList<>();
		messageList.add(message);
		messageList.add(message1);

		Mockito.when(messageRepo.findAll()).thenReturn(messageList);
		assertThat(messageService.getAllMessages()).isEqualTo(messageList);
	}
